  ...
  return (
    <section className="panel-section db-token-activity">
      ...
      <div className="db-token-activity-footer">
        <span>{activeDays ? t("dashboard.activity.activeDays", { count: exactNumber(activeDays), word: t(activeDays === 1 ? "dashboard.word.day" : "dashboard.word.days") }) : t("dashboard.activity.noActivity")}</span>
  ...
  for (const provider of providerUsage?.providers ?? []) {
    for (const metric of provider.account?.metrics ?? []) {
      entries.push({
        ...
        label: metric.label || t("status.usageLimit"),
        metric,
      });
    }
  }
