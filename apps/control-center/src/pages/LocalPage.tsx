  ...
  return (
    <div className="local-page">
      ...
      <section className="panel-section mlx-install-card">
        ...
        {!mlxSupported ? <InlineNotice tone="warning" title={t("local.mlx.appleTitle")}>{mlx?.host?.reason || t("local.mlx.appleBody")}</InlineNotice> : null}
        ...
        {mlxActive ? <DownloadProgress tag={mlxStageLabel(mlxStatus, t)} percent={mlx?.operation?.percent} detail={mlx?.operation?.detail || t("local.mlx.working")} indeterminate={mlx?.operation?.progressMode === "indeterminate"} /> : null}
        {mlxStatus === "error" ? <InlineNotice tone="danger" title={t("local.mlx.errorTitle")}>{mlx?.operation?.error || mlx?.operation?.detail || t("local.mlx.errorBody")}</InlineNotice> : null}
            ...
            <div className="lhc-recommendations">
              {quickPicks.map((model) => (
                <button key={model.tag} type="button" disabled={model.downloadable === false} onClick={() => setInstallRef(model.tag)}>
                  ...
                  <span><strong>{model.displayName || model.label || model.tag}</strong><small>{formatBytesGb(model.sizeGb, t)} · {model.fit || t("local.quick.fitUnknown")}</small></span>
      ...
      <section className="panel-section">
        ...
        {readerDownloadActive ? <DownloadProgress tag={bridge?.download?.tag} percent={bridge?.download?.percent} detail={bridge?.download?.detail || t("local.vision.downloadingReader")} /> : null}
        {localReaders.length ? (
              ...
              return (
                <article className="reader-card" key={reader.tag}>
                  <header>
                    ...
                    <div><strong>{reader.label || reader.displayName || reader.tag}</strong><small>{formatBytesGb(reader.sizeGb, t)} · {reader.accuracy || t("local.vision.untested")}</small></div>
                  ...
                  <p>{reader.note || t("local.vision.defaultNote")}</p>
