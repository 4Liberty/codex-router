            ...
            >
              ...
              {allowances.length ? (
                ...
                <EmptyState
                  ...
                  body={source.message || t("usage.allowances.emptyBody")}
                />
              )}
...
function MetricCard({ source, metric, cardRef, navigationFocused = false }: {
  ...
  const label = metric.label || (metric.kind === "balance" ? t("usage.metric.balance") : t("usage.metric.usageLimit"));
...
function SourceRow({ source, selected, onSelect, t }: {
  ...
  const quota = primary ? `${metricValue(primary, t)} · ${primary.label || t("usage.source.accountMeter")}` : "";
  return (
    <button
      type="button"
      role="listitem"
